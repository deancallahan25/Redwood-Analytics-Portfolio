...
### Setup Instructions: 
The Streamlit Dashboard is deployed to Streamlit Cloud and can be accessed here at this link https://redwood-analytics-project2.streamlit.app/. Once you click the link, you will see a textbox where you can enter a Google API key. If you enter a key, you will then be able to see dynamic commute time estimates based on where you click.  Otherwise, you will be unable to see this information. Additionally, if you provide an API key you will see two visualizations describing the commute times and distances for each mode of transportation. It will also generate a custom recommendation message about whether you should bring a car to campus. 
...
### Data Sources:
- population_addresses_2024_25_messy.csv: a simulated dataset which contains messy student address data. It was cleaned and used in the app as the file "final_address_data.csv".
- simulated_commuter_survey_5yrs_messy.csv: a simulated dataset which contains sample messy survey results describing transportation information from students, staff, and faculty. It was cleaned and used in the app as the file "lon_lat_final_data.csv".
...
- jack-pass-bus-stops.csv: A dataset created by our Domain Expert which describes bus stop locations that students can use for free with their Jack Pass. 

### Team Contributions:
- Jaiden Roe: Tech Lead for the project responsible for coding the Streamlit Dashboard and creation of the visualizations. 
- Zachary Griffith: Domain Expert for the project responsible for finding additional APIs, ensuring dashboard was relevant to the domain, and that the dashboard was user-friendly. 
